    ...
    private static function get_ai_response($query) {
        ...
        switch ($provider) {
            ...
            case 'openrouter':
                ...
                break;
            case 'huggingface':
                if (get_option('smartbot_huggingface_api_key')) {
                    error_log('[SmartBot] Sending query to Hugging Face');
                    $response = HuggingFace_Client::get_response($query);
                } else {
                    error_log('[SmartBot] Hugging Face API key missing');
                }
                break;
            case 'openai':
            default:
                if (get_option('smartbot_assistant_api_key')) {
                    ...
                    $response = OpenAI_Client::get_response($query, $assistant_id);
                } else {
